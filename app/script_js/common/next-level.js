...
export function movingToNextLevel(renderingFunction, typeMove, typeMap, indexCellX, indexCellY) {
    switch (typeMove) {
        case 'long':
            ...
            setTimeout(function () { blackout.classList.add('blackout_behind-canvas'); }, 16000);
            break;
        case 'quickly':
            mapObjects.typeMap = typeMap;
            player.numberOfCompletedFrames = 0;
            ...
            renderingFunction();
            blackout.classList.remove('blackout_active');
            setTimeout(function () { blackout.classList.add('blackout_behind-canvas'); }, 8000);
            break;
        case 'remark':
            cancelAnimationFrame(player.animationId);
            removeEventListener('resize', adaptGameField);
