...
import { blackout, camera, canvas, ctx, field, halfCanvas, keys, mapObjects, level, spacePalette } from "../variables/variables.js";
...
import { adaptGameField, changePositions, levelGeneration, renderingNumberOfLives, renderingNumberOfLivesBoss } from "../common/level-generation.js";
...
import { Demon, CorruptedMadBlick } from "../classes/demons.js";
import { Soul } from "../classes/soul.js";
export function renderingSixthLevel() {
    ...
    if (player.action === 'moving') {
        mapObjects.playerSimulator.push(new PlayerSimulator(player.x, player.y + player.amplitude, player.width));
    }
    if (mapObjects.playerSimulator.length >= 30 || player.action !== 'moving')
        mapObjects.playerSimulator.splice(0, 1);
    //generation shells--------------------------------------------------
    if (keys.mouseLeft.pressed && player.timeWithoutAttacks > 60 && !level.safeZone)
        player.attack = true;
    ...
    if (player.attack && (player.recharge === 0 || player.recharge % 60 === 0) && !level.safeZone) {
        mapObjects.shell.push(new Shell(player.x, player.y, field.sizeCell / 10, player.width, player.mouseX, player.mouseY));
    ...
    if (level.levelStage === 2) {
        ...
        if (player.numberOfCompletedFrames % level.spawnRate === 0 &&
            mapObjects.demon.length < 5)
            mapObjects.demon.push(new Demon(field.sizeCell / 3));
    }
    ...
    mapObjects.stage.forEach(function (stage) {
        if (level.levelStage === 1)
            stage.changePositions();
    });
    mapObjects.shell.forEach(function (shell, index) {
        shell.changePositions();
    });
    mapObjects.shellBlick.forEach(function (shellBlick) {
        shellBlick.changePositions();
    });
    if (level.levelStage === 2)
        mapObjects.demon.forEach(function (demon, index) {
            demon.changePositions();
        });
    if (level.levelStage === 2)
        mapObjects.CorruptedMadBlick.forEach(function (boss) {
            boss.changePositions();
        });
    mapObjects.cubicZirconia.forEach(function (cubicZirconia) {
        if (level.levelStage === 2 && cubicZirconia.condition === 'absent')
    ...
    if (player.bossFightStage)
        collisionOfGameObjects('shell', 'CorruptedMadBlick');
    mapObjects.playerSimulator.forEach(function (playerSimulator) {
        playerSimulator.changePositions();
    });
    player.movementSoul();
    player.numberOfCompletedFrames++;
    mapObjects.particlePlayer.forEach(function (particle, index) {
        if (particle.opacity <= .1)
            mapObjects.particlePlayer.splice(index, 1);
        particle.changePositions();
    });
    mapObjects.particleDemon.forEach(function (particle, index) {
        if (particle.opacity <= .1)
            mapObjects.particleDemon.splice(index, 1);
        particle.changePositions();
    });
    mapObjects.particleBlick.forEach(function (particle, index) {
        if (particle.opacity <= .1)
            mapObjects.particleBlick.splice(index, 1);
        particle.changePositions();
    });
    mapObjects.particleDeadBlick.forEach(function (particle, index) {
        if (particle.opacity <= .1)
            mapObjects.particleDeadBlick.splice(index, 1);
        particle.changePositions();
    });
    ...
    if (player.takesDamage)
        renderingBlackout('damage');
    if (!level.safeZone)
        renderingNumberOfLives();
    if (player.bossFightStage)
        renderingNumberOfLivesBoss('Blick');
