...
import {backpackButton, camera, canvas, ctx, level, mapObjects} from "../variables/variables.js";
...
export function renderingSecondLevel():void {
  ...
  if (player.numberOfCompletedFrames === 0) {
    ...
    objAudio.effects[2].addEventListener('ended', ():void => {
  ...
  mapObjects.floor.forEach((cell:Floor):void => {
    cell.changePositions();
  });

  mapObjects.UniversityInFlesh.forEach((boss:UniversityInFlesh):void => {
  ...
  if (mapObjects.UniversityInFlesh[0].previousStageOfFight === 'transitions' || (mapObjects.UniversityInFlesh[0].previousStageOfFight === 'key phrases' && mapObjects.UniversityInFlesh[0].stageOfKeyPhrase === 0)) mapObjects.UniversityInFlesh[0].showSkipText();
